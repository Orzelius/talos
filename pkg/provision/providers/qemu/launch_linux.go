...
// withNetworkContext on linux creates network namespace, launches CNI and passes control to the next function
...
func withNetworkContext(ctx context.Context, config *LaunchConfig, f func(config *LaunchConfig) error) error {
	...
	err = withCNIOperationLockedNoResult(
		config,
		func() error {
			return cniConfig.DelNetworkList(ctx, config.Network.NetworkConfig, &runtimeConf)
		},
	)
	if err != nil {
	...
	res, err := withCNIOperationLocked(
		...
		func() (types.Result, error) {
			return cniConfig.AddNetworkList(ctx, config.Network.NetworkConfig, &runtimeConf)
		},
	)
	if err != nil {
	...
	cniChain := utils.FormatChainName(config.Network.NetworkConfig.Name, containerID)
