...
// Fields that need to be accessible to the Launch function must be public
// as the struct is json encoded.
...
func Launch() error {
	...
	err = addConfigServerPatch(httpServer, &config)
	if err != nil {
		return err
	}
...
func addConfigServerPatch(configServer inmemhttp.Server, config *LaunchConfig) error {
	configServerAddr, err := getConfigServerAddr(configServer.GetAddr(), *config)
	if err != nil {
		return err
	}
	...
	if strings.Contains(config.KernelArgs, "{TALOS_CONFIG_URL}") {
		...
		config.sdStubExtraCmdlineConfig = fmt.Sprintf(" talos.config=http://%s/config.yaml", configServerAddr)
	}
